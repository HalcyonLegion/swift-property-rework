...
function navigateToLot(lotId) {
    ...
    window.location.href = `/previous_lot_details/${lotId}`;
...
async function updateLots() {
    try {
        ...
        const params = new URLSearchParams({
            ...
            IsShownOnWeb: true,
            OnlineAuctionGroupId: 54479
            // Add more query parameters as needed
        });

        // Call your API endpoint including the query parameters
        const apiUrl = '/api/proxy/online-auctions'; // Proxy endpoint on your Flask server;
        const response = await fetch(`${apiUrl}?${params.toString()}`);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }

        // Existing code where you get the response JSON
      const responseBody = await response.json();

      const auctions = responseBody.Auctions; // Assuming responseBody contains the actual response
        if (!Array.isArray(auctions)) {
            throw new Error('Response is not an array');
        }

        allLots = auctions; // Store the fetched lots for later use
        ...
        if (allLots.length === 0) {
            lotsContainer.innerHTML = '<p>FORTHCOMING LOTS WILL BE ADDED BELOW FOR OUR NEXT AUCTION.</p>';
            return;
        }
...
async function fetchLotData(lotId) {
    try {
        const response = await fetch(`/api/previous_lot_details/${lotId}`);
...
function navigateToLot(lotId) {
    ...
    history.pushState(null, '', `/previous_lot_details/${lotId}`);
...
function renderLotDetails(lot, PreviousLotId, NextLotId) {
    // Assuming lot.Thumbnail is a string URL. If it's an object, use lot.Thumbnail.Url
    const imageHtml = lot.Thumbnail ? `<img src="${lot.Thumbnail}" class="d-block img-fluid" alt="Lot Thumbnail">` : `<p>No image available.</p>`;

    // Map container HTML, initially hidden
    const mapContainerHtml = `<div id="mapContainer" style="display: none;">${createMapIframe(lot.FullAddress)}</div>`;


    // Removing the generation of thumbnails as there's only one image
     ...
     <div class="header-row wide-60 bg-blue mb-3">
            ...
            <div class="row justify-content-md-center">
                ...
                <div class="col-md-5 guide-price txt-700 ml-10">
                    <h4 class="text-white">Sold For:</h4>
    ...
    <div class="container">
         ...
         <div class="row mb-4">
                <!-- Image or No Image Placeholder -->
                    <div id="lotImageCarousel" class="col-12">
                        ${imageHtml}
                    </div>
                    <!-- Map Container -->
                        ${mapContainerHtml}
...
const formattedPrice = new Intl.NumberFormat('en-GB', { style: 'currency', currency: 'GBP' }).format(lot.SalePrice);
